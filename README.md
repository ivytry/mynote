体温：开始时间 结束时间 温度 方式（腋下/口腔）温度计类型（水银/电子） 备注
月经：开始时间 结束时间 痛经程度（1-10）
运动：时间（开始时间/结束时间/时长） 类型（跑步/乒乓球/羽毛球/散步/跑跳） 备注
天气：温度（当前温度/最高温度/最低温度）类型（多云/小雨/阴/雷阵雨）风（1/2/3/4/5/6/7级）
心情：时间（开始时间/结束时间/时长）类型（开心/高兴/难过/心痛/沮丧/悲伤/低落）
sex：时间（开始时间/结束时间/时长）类型（AA/YY）
user：
...
## redux (store actionCreator actionType reducer combineReducers) / react-redux
## propTypes 类型校验
## 虚拟DOM / 虚拟DOM中的diff算法
## react-transition-group / styled-components
## Antd / iconfont
## UI组件 无状态组件 傻瓜组件 容器组件 聪明组件
## redux-thunk / redux-saga
## redux-immutable / axios
